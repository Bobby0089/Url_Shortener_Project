...
export class MyPlansComponent implements OnInit {
  ...
  Api: string = `${environment.apiUrl}/buyplan/viewplan`;
  planArray: any[] = [];
  ...
  viewMyPlan(userId: number | null) {
  if (!userId) {
    console.warn('User ID is not available, cannot fetch plans.');
    return;
  }
  this.http.get<any>(`${this.Api}/${userId}`).subscribe({
    next: (response) => this.planArray = response || [],
